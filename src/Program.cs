...
namespace SesliOkuma
{
    ...
    public sealed class TrayApp : Form
    {
        ...
        bool _barSession;                     // a reading happened; keep the bar open after it finishes

        void SyncBar()
        {
            if (Reader.Active) _barSession = true;
            bool show = _barSession && Settings.ShowReaderBar && !_barHidden;
            if (show)
            {
